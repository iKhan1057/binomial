...
class BinomialExpForNegativeWithoutConstantActivity : BaseActivity() {
    ...
    private fun initviews() {
        ...
        val ast = "(x-y)<sup><small>n</small></sup>"
    ...
    private fun varY(i: Long): String {
        if (i == 1L)
            ...
            return "<font color='green'>y<sup><small>$i</small></sup></font>"
        return ""
    }

    private fun varaX(n: Long): String {
        if (n == 1L)
            return "<font color='green'>x</font>"
        else if (n > 1)
            return "<font color='green'>x<sup><small>$n</small></sup></font>"
        return ""
    }
